...
while(<LIST>){
  ...
  print S1 "cd $result\n";
  print S1 "$lfrcnv -ncpu $threads -bam $bamdir/$info[0]/$info[0].sort.rmdup.bam -vcf $vcfdir/$info[0]/filter.gq.ad.$info[0].gatk4.vcf.gz -phase $phasedir/$info[0]/split -pname hapblock_$info[0]\_XXX -tmp $result/tempData/$info[0] -sp $sp -out $result/$info[0]\n";
  print S1 "mv $result/$info[0]/ALL.200.out.cnv $result/$info[0]/$info[0].CNV.result.xls\n";
